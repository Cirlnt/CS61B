...
public class Repository {
     ...
     */
    private final File objects = Utils.join(GITLET_DIR, "objects");
    /**
     * commits文件夹里面放多个commits，blobs文件夹里面放多个blobs，trees文件里面放
     */
    private final File trees = Utils.join(objects, "trees");
    private final File commits = Utils.join(objects, "commits");
    private final File blobs = Utils.join(objects, "blobs");
    /**
    ...
    private final File branches = Utils.join(GITLET_DIR, "branches");
    /**
     * Stages为暂存区，里面分为addStage和removeStage
     */
    private final File stages = Utils.join(GITLET_DIR, "stages");
    private final File addstages = Utils.join(stages, "addstages");
    private final File removestages = Utils.join(stages, "removestages");
...
//        TreeMap<String, String> splitMap = readObject(Utils.join(branches, splitCommit.getTreeID()), TreeMap.class);
        ...
        // 合并未产生任何更改（且无冲突）→ 输出正常提交错误
        if (!hasConflict && addMap.isEmpty() && rmMap.isEmpty()) {
            System.out.println("No changes added to the commit.");
            return;
        }

        // 提交合并（即使冲突也会提交：树里存的是带冲突标记的内容）
